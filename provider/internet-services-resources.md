...
The IBM Cloud Terraform on {{site.data.keyword.cloud_notm}} Provider plug-in does not support the set up of a region pool for a global load balancer. 
{: note}
...
|`check_regions`|Array|Required| A list of regions (specified by region code) from which to run health checks. If the list is empty, all regions are included, but you must use the Enterprise plan. This is the default setting.|
