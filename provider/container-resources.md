...
|`wait_till`|String|Optional| The cluster creation happens in multi-stages. To avoid the longer wait times for resource execution.This argument in the resource will wait for the specified stage and complete the execution. The default stage value is `IngressReady`. The supported stages are \n **- MasterNodeReady** Resource waits till the master node is ready. \n **- OneWorkerNodeReady** Resource waits till one worker node is in to ready state. \n **- IngressReady** Resource waits till the ingress-host and ingress-secret are available.|No|
